...
```
Setting a admin password

```bash
docker run -d -p 9000:8999 --name portainer6 \
...
docker run -d -p 8000:8000 -p 9000:9000 --name=portainer7 --restart=always -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce